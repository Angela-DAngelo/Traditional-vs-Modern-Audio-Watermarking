...
info = audioinfo(fullfile('',folder,file_name));
 % [sound_stereo,Fs] = audioread(fullfile('../../../00_dataset/',folder, file_name));
 % info = audioinfo(fullfile('../../../00_dataset/',folder, file_name));

%% voglio lavorare a 44KHz
targetFs = 44100;
if Fs ~= targetFs
    sound_stereo = resample(sound_stereo, targetFs, Fs);
    Fs = targetFs;
end
...
while indd+Dati_code.K < length(MARCHIO)
    ...
    % Compute the CRC, but it is not used in this implementation
    data=data_orig;
    % Insert tail bits required for convolutional decoding
    data_pre = data;
