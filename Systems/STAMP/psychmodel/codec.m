...
pcm = wavread('2ch_44.1kHz_jazz.wav');
pcm = pcm(1e6+1:1e6+1e5,:);
...
%   q_left=mdct_quant(mdct_left,ceil(log2(steps_l)));
%   q_right=mdct_quant(mdct_right,ceil(log2(steps_r)));
%  lbits=lbits+numbits_l;
%  rbits=rbits+numbits_r;
  %q_right=mdct_quant(mdct_right);
  %take imdct
  gmask_l=(mdct_left.^2.*gmask_l)./(abs(fft_left(1:256))).^2;
  gmask_r=(mdct_left.^2.*gmask_r)./(abs(fft_right(1:256))).^2;
  convert_l=sqrt(10.^((gmask_l-90.302)./10));
  convert_r=sqrt(10.^((gmask_r-90.302)./10));
