...
%
%      "Model-Driven and Learning-Based Audio
%       Watermarking: An Experimental Comparison"
%
...
%===========================PARAMETERS=====================================
% DATASET------------------------------------------------------------------
dataset_name='PhonemeDF';
%dataset_name_short=[dataset_name '_short'];
dataset_path = fullfile('',dataset_name); %originale
dataset_path=['E:\Ricerca\2026\audio_watermarking\AudioWatermarking\00_final_test\00_dataset\',dataset_name];
%dataset_path_short = ['E:\Ricerca\2026\audio_watermarking\AudioWatermarking\00_final_test\00_dataset\',dataset_name_short];
...
% case 3: select desidered bits number in set_param.m (default 40 bits)
...
% % salvo audio short - per la qualità
...
% [ ~, name, ext] = fileparts(file_name);
% audiowrite(fullfile(dataset_path_short, file_name), input_sound,16000);
...
total_embedding_time = total_embedding_time + toc(t_embed);
mem_info = memory;
...
audiowrite(fullfile(['watermarked/',dataset_name,'/', marked_file_name]), output_sound(1).vals,44100);
...
% audioFiles = dir(fullfile('attacked/',dataset_name, '*.wav'));
...
%         [sound_stereo,Fs] = audioread(fullfile('attacked/',dataset_name, audioName));
%         fprintf('Processing: %s\n', audioName);
% 
...
total_decoding_time = total_decoding_time + toc(t_decode);

mem_info = memory;
memory_used_MB = mem_info.MemUsedMATLAB / 1024^2;
...
processed_files = length(file_list);

complexityExcel = ['computational_complexity40_' dataset_name '.xlsx'];

header = { ...
    'processed files', ...
    'total embedding time [s]', ...
    'total decoding time [s]', ...
    'mean embedding per file [s]', ...
    'mean decoding per file [s]', ...
    'peak RAM usage [MB]'};
...
values = { ...
    ...
    total_decoding_time, ...
    total_embedding_time/processed_files, ...
    total_decoding_time/processed_files, ...
    peak_memory_MB};

writecell(header, complexityExcel, ...
    'Sheet',1,'Range','A1');

writecell(values, complexityExcel, ...
    'Sheet',1,'Range','A2');
